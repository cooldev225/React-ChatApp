        ...
        <div class="col-12">
            <div class="login-contain-main">
                <div class="left-page">
                    ...
                    <div class="login-content-header"><img src="/chat/images/logo/landing-logo.png" alt="images"></div>
                    ...
                    <form class="form1" action="/register" method="POST">
                        ...
                        <div class="form-group" {{ $errors->has('username') ? 'has-error' : '' }}>
                            ...
                            <input class="form-control" id="uname" type="text" name="username" placeholder="Demo@123" value="{{ old('username') }}">
                            <span class="text-danger">{{ $errors->first('username') }}</span>
                        </div>
                        <div class="form-group" {{ $errors->has('email') ? 'has-error' : '' }}>
                            <label class="col-form-label" for="inputEmail3">Email Address</label>
                            <input class="form-control" id="inputEmail3" type="email" name="email" placeholder="Demo@123gmail.com" value="{{ old('email') }}">
                            <span class="text-danger">{{ $errors->first('email') }}</span>                            
                        </div>
                        <div class="form-group" {{ $errors->has('password') ? 'has-error' : '' }}>
                            <label class="col-form-label" for="inputPassword3">Password</label><span> </span>
                            <input class="form-control" id="inputPassword3" type="password" name="password" placeholder="Password" value="{{ old('password') }}">
                            <span class="text-danger">{{ $errors->first('password') }}</span>                            
                        </div>
                        <div class="form-group">
                            <div class="buttons"><a class="btn btn-primary button-effect signpbtn" href="#" onclick="$('.form1').submit();">Signup</a></div>
                        </div>
                        <div class="medialogo">
                            <ul>
